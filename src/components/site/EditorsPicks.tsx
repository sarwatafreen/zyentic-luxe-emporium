...
export function EditorsPicks() {
  ...
  return (
    <section id="picks" className="border-b border-foreground py-20 md:py-28">
        ...
        <div className="flex items-end justify-between mb-12 md:mb-16">
          ...
          <div className="hidden md:flex gap-3">
            <button
              onClick={() => scroll("l")}
              aria-label="Previous"
              className="w-12 h-12 border border-foreground flex items-center justify-center hover:bg-foreground hover:text-background transition"
            >
      ...
      <div
        ref={scrollRef}
        className="flex gap-5 overflow-x-auto scroll-smooth px-6 md:px-12 pb-4 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden"
      >
        {categories.map((c, i) => (
          <motion.a
            key={c.name}
            ...
            className="group relative shrink-0 w-[280px] md:w-[360px] aspect-[3/4] border border-foreground overflow-hidden block"
          >
            <img
              src={c.image}
              alt={c.name}
